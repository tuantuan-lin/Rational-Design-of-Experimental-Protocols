...
def data_read():
    ...
    y_tem.astype('float')
    y_def = np.ravel(y_def)
    y_def.astype('float')

    return x0, x1, y_tem, y_def
...
if __name__ == '__main__':
    ...
    for u in range(0, m):
        x11, y11 = resample(x1, y_def, replace=True, n_samples=n, random_state=u)
        predict_y = model_pre(x11, y11
                              , 'xgboost'
                              , r'xgboost_DC.pkl'
                              , r'DC_models_params.npy'
                              , space_pre)
